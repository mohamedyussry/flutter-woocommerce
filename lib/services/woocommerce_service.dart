...
class WooCommerceService {
  ...
  Future<List<dynamic>> getProductVariations(int productId) async {
    try {
      final response = await _client.get(
      ...
      debugPrint('Error fetching product variations: $e');
      rethrow;
    }
  }
  ...
  Future<List<dynamic>> getProductReviews(int productId) async {
    try {
      final uri = Uri.parse('$_baseUrl/wp-json/wc/v3/products/reviews').replace(
        queryParameters: {
  ...
  Future<List<dynamic>> getRelatedProducts(int productId, {int perPage = 4}) async {
    try {
      final product = await getProductDetails(productId);
      final List<dynamic> relatedIds = product['related_ids'] ?? [];
      
      if (relatedIds.isEmpty) return [];

      final uri = Uri.parse('$_baseUrl/wp-json/wc/v3/products').replace(
        queryParameters: {
          'include': relatedIds.take(perPage).join(','),
        },
      );
      final response = await _client.get(
        uri,
  ...
  Future<List<dynamic>> getFeaturedProducts() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('فشل في جلب المنتجات المميزة');
      }
    } catch (e) {
  ...
  Future<List<dynamic>> getNewProducts() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('فشل في جلب المنتجات الجديدة');
      }
    } catch (e) {
      debugPrint('Error fetching new products: $e');
      rethrow;
    }
  }
